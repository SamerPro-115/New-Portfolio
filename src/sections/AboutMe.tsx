import { motion } from "framer-motion";
import { ArrowDown, ArrowRight } from "lucide-react";
import { useEffect, useState } from "react";

export function AboutMe() {
  const [expandedImage, setExpandedImage] = useState<number | null>(null);
  const [isMid, setIsMid] = useState<boolean>(false)

  const images = [
    { src: "/madrid.png", alt: "madrid", id: 0 },
    { src: "/manga.jpg", alt: "manga", id: 1 },
    { src: "/Rocket.png", alt: "rocket league", id: 2 }
  ];

  const handleImageClick = (imageId: number, event: React.MouseEvent) => {
    event.stopPropagation();
    
    if (expandedImage === imageId) {
      setExpandedImage(null);
    } else {
      setExpandedImage(imageId);
    }
  };

  const handleContainerClick = () => {
    setExpandedImage(null);
  };

  const screenWidth = window.screen.width;


  useEffect(() => {
    if(screenWidth < 1024) setIsMid(true)
  }, [screenWidth])

  return (
    <div
      className="min-h-screen flex lg:flex-row flex-col justify-center"
      onClick={handleContainerClick}
    >
      <div className="text flex flex-1 bg-black  justify-center items-center">
        <div className="flex flex-col w-2/3 lg:mt-0 lg:mb-0 sm:mt-30 sm:mb-30 mt-10 mb-10">
          <motion.h1
            initial={{ opacity: 0, x: -100 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            viewport={{ once: false, amount: 0.3 }}
            className="text-4xl lg:text-6xl font-bold text-white mb-6 leading-tight"
          >
            About Me
          </motion.h1>

          <motion.p
            initial={{ opacity: 0, x: -100 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, delay: 0.4 }}
            viewport={{ once: false, amount: 0.3 }}
            className="text-lg lg:text-xl text-gray-300"
          >
            I'm a university student in the Faculty of IT. Outside of coding, I
            have passions that inspire me and keep life exciting
            {isMid ? (
              <ArrowDown className="inline" />
            ) : (
              <ArrowRight className="inline" />
            )}
          </motion.p>
        </div>
      </div>

      <div className="bg-black flex-1 justify-center items-center flex gap-[1vmin]">
        {images.map((image,index) => (
        <motion.img
   initial={{ rotateY: 90, opacity: 0 }}
    whileInView={{ rotateY: 0, opacity: 1 }}
    transition={{ 
      duration: 0.8, 
      delay: index * 0.15,
      ease: "easeOut"
    }}
    viewport={{ once: true, amount: 0.1,  margin: "0px 0px -100px 0px"   }}
  key={image.id}
  src={image.src}
  className={`object-cover lg:h-screen md:h-[70vh] h-[50vh] overflow-hidden 
    transition-all duration-500 hover:grayscale-0 hover:brightness-110 hover:flex-[6]
    ${
      expandedImage === image.id
        ? "flex-[6] grayscale-0 brightness-110"
        : expandedImage !== null
        ? "flex-1 grayscale brightness-75"
        : "flex-1 grayscale brightness-75 hover:grayscale-0 hover:brightness-110 hover:flex-[6]"
    }`}
  alt={image.alt}
  onClick={(event) => handleImageClick(image.id, event)}
/>
        ))}
      </div>
    </div>
  );
}